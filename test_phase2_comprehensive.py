...
async def test_phase2_components():
    ...
    try:
        ...
        for i, suggestion in enumerate(suggestions, 1):
            print(f"      {i}. {suggestion['title']} (confidence: {suggestion.get('confidence', 0):.2f})")
    ...
    try:
        ...
        print(f"   🔗 Integration test: {len(current_suggestions)} suggestions generated")
        
        # Test analytics
        analytics = await anticipation_engine.get_analytics_summary()
        print(f"   📈 Total activities tracked: {analytics['activity_tracking']['total_activities']}")
