...
def test_plugin_manager_integration():
    ...
    try:
        ...
        class MockWidget:
            ...
            def clicked(self): return MockWidget()
            def connect(self, *args): pass
            def stateChanged(self): return MockWidget()
            def itemClicked(self): return MockWidget()
        ...
        class MockModules:
            ...
            QBrush = MockWidget
            Qt = type('Qt', (), {'ItemDataRole': type('ItemDataRole', (), {'UserRole': 0}),
                                'CheckState': type('CheckState', (), {'Checked': type('Checked', (), {'value': 2})})})()
            ModernTheme = type('ModernTheme', (), {
                'SURFACE': '#000', 'BORDER': '#111', 'TEXT_PRIMARY': '#fff',
                'TEXT_SECONDARY': '#ccc', 'PRIMARY': '#00f', 'SUCCESS': '#0f0',
                'WARNING': '#ff0'
            })
        ...
        sys.modules['src.neurocode.ui.components.utils.qt_imports'] = qt_module
        sys.modules['..utils.qt_imports'] = qt_module

        # Mock other dependencies
        sys.modules['..cards'] = type('cards', (), {'ModernCard': MockWidget})
        sys.modules['..theme'] = type('theme', (), {'ModernTheme': MockModules.ModernTheme})
...
if __name__ == "__main__":
    ...
    if success:
        print("\n🎉 All tests passed! The plugin manager should now show your plugins in Neuroplex.")
    else:
