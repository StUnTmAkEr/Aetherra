...
async def test_intelligence_stack():
    ...
    try:
        ...
        print(f"   📊 System Workflows: {workflow_health:.1f}% ({active_workflows} active)")
    ...
    try:
        workflow_result = await intelligence_stack.run_intelligence_workflow("goal_autopilot")
        if workflow_result.get('success', False):
            print(f"✅ Workflow execution successful")
            print(f"   Result: {workflow_result.get('result', 'No result')}")
        else:
            print(f"❌ Workflow execution failed: {workflow_result.get('error', 'Unknown')}")
    except Exception as e:
    ...
    try:
        ...
        if reflection.get('status') != 'error':
            ...
            print(f"❌ System Reflection failed: {reflection.get('message', 'Unknown')}")
    except Exception as e:
    ...
    try:
        ...
        if final_health >= 80:
            print(f"🟢 EXCELLENT: Intelligence Stack is fully operational ({final_health:.1f}%)")
        elif final_health >= 60:
            print(f"🟡 GOOD: Intelligence Stack is mostly operational ({final_health:.1f}%)")
        else:
            print(f"🔴 NEEDS ATTENTION: Intelligence Stack requires maintenance ({final_health:.1f}%)")
        
        print(f"\n📊 Final Statistics:")
        print(f"   • Intelligence Components: {sum(1 for x in final_status['intelligence_layer']['status'].values() if x)}/6")
        print(f"   • Active Workflows: {final_status['system_workflows']['active_count']}/5")
        print(f"   • Active Modules: {final_status['system_modules']['active_count']}/6")
