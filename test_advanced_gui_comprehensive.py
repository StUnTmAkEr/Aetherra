...
def test_enhanced_analytics():
    ...
    try:
        ...
        from lyrixa.gui.enhanced_analytics import AgentUsageMetrics, ProductivityMetrics
    ...
    try:
        ...
        feedback_actions = [
            ...
            (
                ...
                "test_sug_001",
                {
                    "edited_text": "Break down complex functions into smaller, focused units."
                },
            ),
            (
                "rate_interaction",
                "",
                {"rating": 4.0, "comment": "Very helpful suggestion!"},
            ),
        ]

        for action_type, item_id, data in feedback_actions:
            result = feedback_interface.handle_feedback_action(
                action_type, item_id, data
            )
        ...
        print(
            f"✅ Learning insights retrieved: {insights['learning_stats']['total_feedback_items']} feedback items"
        )
        ...
        print(
            f"✅ Personality profile retrieved: intervention_frequency={profile.intervention_frequency:.2f}"
        )

        adaptation_summary = learning_engine.get_adaptation_summary()
        print(
            f"✅ Adaptation summary retrieved: {adaptation_summary['status'] if 'status' in adaptation_summary else 'has_data'}"
        )
...
def test_cross_component_integration():
    ...
    try:
        ...
        feedback_interface.handle_feedback_action(
            "positive_feedback", "integration_test", {}
        )
        ...
        web_interface.notification_manager.user_preferences[
            "max_notifications_per_hour"
        ] = max(1, int(personality.intervention_frequency * 5))
        ...
        for (
            feedback
        ) in feedback_interface.learning_engine.feedback_collector.feedback_history[
            -5:
        ]:
            memory_update = {
