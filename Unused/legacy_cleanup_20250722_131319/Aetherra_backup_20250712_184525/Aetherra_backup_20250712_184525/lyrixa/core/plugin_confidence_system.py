...
class SafetyAnalysis:
    ...
    UNSAFE_IMPORTS = {
        ...
        "open",
        "file",  # File operations need scrutiny
        "urllib.request",
        ...
        "ctypes",
        "cffi",  # Low-level system access
        "threading.Thread",
        "multiprocessing",  # Threading concerns
    ...
    def _calculate_risk_level(self, safety_report: Dict):
        ...
        has_critical = any(
            issue["severity"] == "CRITICAL" for issue in safety_report["issues"]
        )
...
class RuntimeMetrics:
    ...
    def get_plugin_metrics(
        self, plugin_name: str, days_back: int = 30
    ) -> Dict[str, Any]:
...
class PluginScorer:
    ...
    def analyze_plugin(
        self, plugin_name: str, plugin_code: str, plugin_path: Optional[Path] = None
    ) -> Dict[str, Any]:
        """
        ...
        safety_analysis = self.safety_analyzer.analyze_code_safety(
        ...
        confidence_score = self._calculate_confidence_score(
            safety_analysis, runtime_metrics
        )
        ...
        self._update_confidence_database(
            plugin_name, confidence_score, safety_analysis, runtime_metrics
        )
    ...
    def _calculate_confidence_score(
        self, safety_analysis: Dict, runtime_metrics: Dict
    ) -> float:
        """
    ...
    def _update_confidence_database(
        self,
        plugin_name: str,
...
def get_plugin_confidence_warning(
    plugin_name: str, confidence_score: float, risk_level: str
) -> Optional[str]:
...
def should_block_plugin_execution(
    risk_level: str, confidence_score: float, user_override: bool = False
) -> bool:
