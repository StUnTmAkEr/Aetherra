...
class AnticipationEngine:
    ...
    async def _suggest_task_optimizations(
        ...
        if len(activities) >= 5:
            ...
            if len(set(recent_types[-3:])) == 1 and all(
                d < 300 for d in recent_durations[-3:]
            ):
                suggestion = Suggestion(
                    suggestion_id=str(uuid.uuid4()),
        ...
        if keywords:
            ...
            try:
                related_memories = await self.memory_system.semantic_search(
                    " ".join(keywords), top_k=3, min_confidence=0.6
                )
                ...
                logging.warning(
                    f"Error searching memories for knowledge discovery: {e}"
                )
    ...
    def _should_show_suggestion(self, suggestion: Suggestion) -> bool:
        ...
        for existing in self.active_suggestions.values():
            if (
                ...
                and (datetime.now() - existing.created_at).total_seconds()
    ...
    async def get_active_suggestions(self) -> List[Suggestion]:
        ...
        expired_ids = [
            sid
            for sid, suggestion in self.active_suggestions.items()
    ...
    async def respond_to_suggestion(
        self, suggestion_id: str, response: str, was_helpful: bool
    ) -> None:
    ...
    async def _learn_from_feedback(
        self, suggestion: Suggestion, was_helpful: bool
    ) -> None:
    ...
    def _matches_pattern_start(
        self, recent_types: List[str], pattern_sequence: List[str]
    ) -> bool:
        ...
        for i in range(min(3, len(recent_types))):
            if (
                len(pattern_sequence) > i
                and recent_types[-(i + 1)] == pattern_sequence[i]
            ):
                continue
            else:
                return False
    ...
    def _predict_next_step(
        ...
        for i, activity_type in enumerate(reversed(recent_types)):
            if (
                matching_index < len(pattern_sequence)
                and activity_type == pattern_sequence[matching_index]
            ):
                matching_index += 1
            else:
                break
        ...
        for context in contexts:
            for key, value in context.items():
                if isinstance(value, str) and len(value) > 3:
                    ...
                    keywords.extend(
                        [str(item).lower() for item in value if len(str(item)) > 3]
                    )
    ...
    async def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        return {
            "anticipation_performance": {
                ...
                "helpfulness_rate": (suggestion_stats[2] or 0)
                / max(1, suggestion_stats[0] or 1),
...
async def test_anticipation_engine():
    ...
    activities = [
        ...
        (
            "research",
            {"topic": "machine_learning", "source": "documentation"},
            600,
            0.6,
        ),
    ...
    if suggestions:
        await engine.respond_to_suggestion(
            suggestions[0].suggestion_id, "accepted", was_helpful=True
        )
    ...
    print(
        f"   📊 Total activities: {analytics['activity_tracking']['total_activities']}"
    )
    print(
        f"   📊 Total suggestions: {analytics['anticipation_performance']['total_suggestions']}"
    )
