...
class AetherraHousekeeping:
    ...
    def __init__(self, project_root: str):
        ...
        self.demo_patterns = ["demo_*.py", "*_demo.py", "simple_*.py", "minimal_*.py"]

        self.temp_patterns = ["debug_*.py", "quick_*.py", "fix_*.py", "*.tmp", "*.temp"]
                ...
                try:
                    ...
                    if size_mb > 10:  # Files larger than 10MB
                        analysis["large_files"].append(
                            {"file": str(relative_path), "size_mb": round(size_mb, 2)}
                        )
                except Exception:
                    pass
            ...
            if dir_path.name not in self.important_dirs:
                try:
                    if not any(dir_path.iterdir()):  # Directory is empty
                        dir_path.rmdir()
                        self.cleanup_report["directories_cleaned"].append(
    ...
    def clean_pycache(self) -> int:
        ...
        for pycache_dir in self.project_root.rglob("__pycache__"):
            try:
                ...
                self.cleanup_report["directories_cleaned"].append(
                    str(pycache_dir.relative_to(self.project_root))
                )
                removed_count += 1
            except Exception as e:
        ...
        for dir_name, patterns in organized_dirs.items():
            ...
            for pattern in patterns:
                for file_path in self.project_root.glob(pattern):
                    if (
                        file_path.is_file()
                        and file_path.parent == self.project_root
                        and file_path.name not in self.keep_files
                    ):
                        try:
    ...
    def perform_comprehensive_cleanup(self) -> Dict:
        ...
        self.cleanup_report["operations_performed"].append(
            "comprehensive_cleanup_started"
        )
        ...
        self.cleanup_report.update(
            {
                ...
                "total_files_processed": test_moved
                + demo_moved
                + temp_removed
                + reports_moved,
            }
    ...
    def generate_cleanup_report(self) -> str:
        ...
        report_path = (
            self.project_root
            / f"housekeeping_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
