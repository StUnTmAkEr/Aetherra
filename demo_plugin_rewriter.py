...
def demonstrate_plugin_rewriter():
    ...
    try:
        ...
        with patch("openai.OpenAI") as mock_openai_class:
            ...
            print("🔧 DEMONSTRATION 2: Plugin Refactoring")
            print("-" * 40)

            mock_response.choices[0].message.content = mock_responses["refactor"]
            mock_client.chat.completions.create.return_value = mock_response

            refactor_result = rewriter.refactor_plugin(
                "data_processor", "add type hints and improve code quality"
            )
            ...
            if len(versions) >= 2:
                diff_result = rewriter.diff_plugin_versions(
                    "data_processor", versions[1], versions[0]
                )
                print("📊 Differences between last two versions:")
                print("```diff")
                print(
                    diff_result[:500] + "..." if len(diff_result) > 500 else diff_result
                )
                print("```")
                print()
            ...
            if len(versions) >= 1:
                rollback_result = rewriter.rollback_plugin(
                    "data_processor", versions[-1]
                )  # Rollback to oldest
