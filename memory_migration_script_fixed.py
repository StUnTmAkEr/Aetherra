    ...
    async def _migrate_advanced_vector_memory(
        ...
        for memory in memories:
            ...
            if result.success:
                ...
                # Add migration metadata if fragment_id exists
...
"""
        ...
        for result in successful:
            source_name = (
                Path(result.get("source", "unknown")).name
                if result.get("source")
                else "unknown"
            )
            result_type = result.get("type", "unknown")
            fragments = result.get("fragments_created", 0)
            report += f"• {source_name} ({result_type}) → {fragments} fragments\\n"
...
"""
            for result in failed:
                ...
                report += f"• {source_name}: {error_msg}\\n"
