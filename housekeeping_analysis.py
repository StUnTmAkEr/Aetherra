...
class ProjectHousekeeping:
    ...
    def analyze_file(self, file_path):
        ...
        if any(
            keyword in file_str
            for keyword in ["test_", "test\\", "tests\\", "_test.py", "testing\\"]
        ):
        ...
        # Backup files
        if any(
            keyword in file_str for keyword in ["backup", ".bak", ".old", "archive"]
        ):
        ...
        if any(
            keyword in file_str
            for keyword in [".tmp", ".temp", "__pycache__", ".pyc", ".log"]
        ):
            ...
            if len(paths) > 1:
                ...
                if len(non_test_paths) > 1:
                    self.analysis["duplicates"].append(
                        {"filename": filename, "paths": paths, "count": len(paths)}
                    )
    ...
    def generate_recommendations(self):
        ...
        old_docs = [
            f
            for f in self.analysis["documentation_files"]
    ...
    def print_summary(self):
        ...
        sorted_extensions = sorted(
            self.analysis["by_extension"].items(), key=lambda x: x[1], reverse=True
        )[:10]
        ...
        print(f"\n📂 TOP DIRECTORIES:")
        sorted_dirs = sorted(
            self.analysis["by_directory"].items(), key=lambda x: x[1], reverse=True
        )[:10]
