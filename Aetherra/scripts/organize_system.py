...
Examples:
  ...
  %(prog)s optimize --execute      # Execute safe optimizations
    ...
    action_group.add_argument('--scan', action='store_true', 
                            help='Scan project files and build registry')
    action_group.add_argument('--analyze', action='store_true',
                            help='Perform comprehensive system analysis')
    action_group.add_argument('--monitor', action='store_true',
                            help='Start real-time file system monitoring')
    action_group.add_argument('--optimize', action='store_true',
                            help='Execute optimization suggestions')
    ...
    parser.add_argument('--config', 
                       help='Configuration file for monitoring daemon')
        ...
        if args.scan:
            ...
            if results['executed']:
                ...
                for item in results['executed']:
                    print(f"    • {item.get('type', 'Unknown')}: {item.get('reason', 'No details')}")
