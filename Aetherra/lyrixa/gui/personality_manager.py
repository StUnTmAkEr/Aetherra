...
class PersonalityManager:
    ...
    def __init__(self, config_path: str = "lyrixa_personality.json"):
        ...
        self.current_personality = (
            "aetherra_core"  # Default to manifesto-aligned personality
        )
        self.custom_traits = {}
    ...
    def get_self_introduction(self, context: str = "general") -> str:
        ...
        base_intro = current.get("response_style", {}).get(
    ...
    def get_manifesto_response(self, query_type: str, user_question: str = "") -> str:
        ...
        return responses.get(
            query_type,
            "I embody the Aetherra Manifesto's vision of AI-native computing. What specific aspect would you like to explore? Try asking about 'what is aetherra', 'why different', or 'future vision'.",
        )
    ...
    def get_manifesto_context_hook(self, user_input: str) -> str:
        ...
        if any(
            ...
            phrase in user_lower
            for phrase in ["manifesto", "principles", "philosophy", "vision", "mission"]
        ):
            return self.get_manifesto_response("manifesto_core", user_input)

        elif any(
            phrase in user_lower
            for phrase in ["future", "roadmap", "next", "where is this going"]
        ):
            return self.get_manifesto_response("future_vision", user_input)

        elif any(
            phrase in user_lower
            for phrase in ["how to start", "getting started", "begin", "try aetherra"]
        ):
            return self.get_manifesto_response("getting_started", user_input)

        elif any(
            phrase in user_lower
            for phrase in ["ai os", "operating system", "beyond programming"]
        ):
    ...
    def summarize_manifesto_for_user(self) -> str:
        ...
        if self.current_personality != "aetherra_core":
            return (
                "Switch to 'Aetherra Core' personality for detailed manifesto insights."
            )
