...
# --- Real Fractal and Quantum Imports ---
from .compression_analyzer import MemoryCompressionAnalyzer
from .fractal_encoder import FractalEncoder

try:
    from .fractal_hierarchies import FractalHierarchyManager
except (ImportError, AttributeError):
    FractalHierarchyManager = None
    print("⚠️ FractalHierarchyManager not available")
try:
    from .fractal_replay_engine import FractalReplayEngine
except ImportError:
    FractalReplayEngine = None
    print("⚠️ FractalReplayEngine not available")
try:
    from .quantum_memory_bridge import QuantumMemoryBridge
except ImportError:
    QuantumMemoryBridge = None
    print("⚠️ QuantumMemoryBridge not available")
try:
    from .quantum_memory_integration import QuantumEnhancedMemoryEngine
except ImportError:
    QuantumEnhancedMemoryEngine = None
    print("⚠️ QuantumEnhancedMemoryEngine not available")
...
class QFACDashboard:
    ...
    def __init__(self, analyzer: MemoryCompressionAnalyzer):
        ...
        # --- Fractal and Quantum Engine Wiring ---
        # Fractal/Quantum modules are optional, handle gracefully
        try:
            self.fractal_encoder = FractalEncoder()
            print("✅ FractalEncoder loaded")
        except Exception as e:
            self.fractal_encoder = None
            print(f"⚠️ FractalEncoder not available: {e}")

        if FractalHierarchyManager:
            try:
                self.fractal_hierarchy = FractalHierarchyManager()
                print("✅ FractalHierarchyManager loaded")
            except Exception as e:
                self.fractal_hierarchy = None
                print(f"⚠️ FractalHierarchyManager not available: {e}")
        else:
            self.fractal_hierarchy = None

        if FractalReplayEngine and self.fractal_encoder is not None:
            try:
                self.fractal_replay = FractalReplayEngine(self.fractal_encoder)
                print("✅ FractalReplayEngine loaded")
            except Exception as e:
                self.fractal_replay = None
                print(f"⚠️ FractalReplayEngine not available: {e}")
        else:
            self.fractal_replay = None

        if QuantumMemoryBridge:
            try:
                self.quantum_bridge = QuantumMemoryBridge()
                print("✅ QuantumMemoryBridge loaded")
            except Exception as e:
                self.quantum_bridge = None
                print(f"⚠️ QuantumMemoryBridge not available: {e}")
        else:
            self.quantum_bridge = None

        if QuantumEnhancedMemoryEngine:
            try:
                self.quantum_engine = QuantumEnhancedMemoryEngine()
                print("✅ QuantumEnhancedMemoryEngine loaded")
            except Exception as e:
                self.quantum_engine = None
                print(f"⚠️ QuantumEnhancedMemoryEngine not available: {e}")
        else:
            self.quantum_engine = None
    ...
    async def _start_interactive_dashboard(self):
        ...
        try:
            ...
            print(
                "⚠️ matplotlib.animation.FuncAnimation not available - falling back to text mode"
            )
            await self._start_text_dashboard()
            return
    ...
    async def _start_web_dashboard(self):
        ...
        try:
            ...
            # Prefer real quantum engine if available
            quantum_engine = self.quantum_engine if self.quantum_engine else None
            if quantum_engine:
                print("✅ QuantumEnhancedMemoryEngine wired to dashboard")
            else:
                print("⚠️ No quantum engine available - dashboard will show mock data")
            web_dashboard = QuantumWebDashboard(quantum_engine, port=8080)
            dashboard_url = await web_dashboard.start()
            print("Quantum Web Dashboard started!")
            print(f"📱 Open in browser: {dashboard_url}")
            print("💡 Features:")
            print("   • Real-time quantum coherence monitoring")
            print("   • Quantum operation statistics")
