...
class ProjectProtection:
    ...
    def get_last_backup_time(self) -> str:
        ...
        try:
            ...
            return datetime.datetime.fromtimestamp(latest.stat().st_mtime).strftime("%Y-%m-%d %H:%M:%S")
...
def main():
    ...
    if command == "status":
        ...
        print(f"  Protected Files: {status['protected_files_existing']}/{status['protected_files_count']}")
