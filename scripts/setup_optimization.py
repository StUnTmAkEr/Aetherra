...
def main():
    ...
    for package in ai_packages:
        choice = input(f"Install {package.split('>=')[0]}? This may be a large download. (y/N): ").lower()
        if choice in ['y', 'yes']:
            run_command(f"pip install {package}", f"Installing {package.split('>=')[0]}")
        else:
    ...
    test_imports = [
        ...
        ("rich", "from rich.console import Console; Console().print('Rich working!', style='bold green')"),
    ...
    print("\\n🎨 Testing GUI import...")
    try:
        subprocess.run([sys.executable, "-c", "from ui.neuroplex_gui import main; print('✅ GUI ready!')"], 
                      check=True, capture_output=True, cwd=Path.cwd())
        print("✅ NeuroCode GUI is ready to launch!")
    except subprocess.CalledProcessError as e:
        print("⚠️  GUI import test failed - but this may be normal if display is not available")
