...
class PluginRegistryClient:
    ...
    def __init__(self, registry_url: str = "https://registry.neurocode.org/api/v1"):
        ...
        self.session.headers.update({
            "User-Agent": "NeuroCode-Plugin-Manager/3.0.0",
...
class PluginSecurityManager:
    ...
    GPL_COMPATIBLE_LICENSES = [
        ...
        "LGPL-3.0", "LGPL-3.0-or-later", "LGPL-2.1", "LGPL-2.1-or-later",
        "BSD-3-Clause", "BSD-2-Clause", "MIT", "Apache-2.0", 
        "MPL-2.0", "ISC", "X11", "Public-Domain"
    ]
    
    DANGEROUS_PERMISSIONS = [
        "system_access", "network_unrestricted", "file_system_write",
        "process_execution", "registry_modification"
    ]
    ...
    def scan_plugin_code(self, plugin_path: Path) -> Dict[str, Any]:
        ...
        for neuro_file in plugin_path.glob("**/*.neuro"):
            ...
            dangerous_patterns = [
                "system_execute", "file_delete", "network_raw",
                "memory_direct", "process_kill", "registry_write"
            ]
...
class EnhancedPluginManager:
    ...
    def __init__(self, plugins_dir: str = None, registry_url: str = None):
        # Setup directories
        ...
        self.registry = PluginRegistryClient(registry_url) if registry_url else PluginRegistryClient()
        ...
        logger.info(f"🔌 Enhanced NeuroCode Plugin Manager initialized")
        ...
        try:
            ...
            if not skip_security:
                if not self.security.validate_license(plugin_info.manifest.license):
                    logger.error(f"❌ Plugin license '{plugin_info.manifest.license}' is not GPL-3.0 compatible")
                    return False
                
                permissions = self.security.validate_permissions(plugin_info.manifest.security_permissions)
                ...
                if not skip_security:
                    ...
                    if not security_report["safe"]:
                        ...
                        for threat in security_report["threats_found"]:
                            logger.error(f"   {threat['file']}: {threat['pattern']} ({threat['severity']})")
                        return False
...
def main():
    ...
    install_parser.add_argument("--skip-security", action="store_true", help="Skip security validation")
    ...
    if args.command == "install":
        ...
        if plugins:
            ...
            for plugin in plugins:
                ...
                print(f"{status} {plugin.manifest.name} v{plugin.manifest.version} ({plugin.manifest.category})")
                print(f"    {plugin.manifest.description}")
                
                if args.detailed:
                    print(f"    Author: {plugin.manifest.author} | License: {plugin.manifest.license}")
        ...
        if results:
            ...
            for plugin in results:
                ...
                print(f"{status_icon} {plugin.manifest.name} v{plugin.manifest.version} ({plugin.manifest.category})")
                print(f"    {plugin.manifest.description}")
                print(f"    Author: {plugin.manifest.author} | Downloads: {plugin.download_count:,} | {rating_stars}")
                if plugin.manifest.keywords:
                    print(f"    Keywords: {', '.join(plugin.manifest.keywords)}")
                print()
        else:
