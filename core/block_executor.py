...
class BlockExecutor:
    ...
    def _execute_function_definition(self, lines: List[str], start: int, context: ExecutionContext, result: List[str]) -> int:
    ...
    def _evaluate_condition(self, condition: str, context: ExecutionContext) -> bool:
        ...
        if 'memory.pattern' in condition:
            ...
            pattern_match = re.search(r'memory\.pattern\(["\'](.*?)["\'](?:,\s*frequency=["\'](.*?)["\']\))?\)', condition)
            if pattern_match:
