...
class NeuroCodeParser:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            'memory_pattern': re.compile(r'^\s*memory\.pattern\("([^"]+)"(?:,\s*frequency="([^"]+)")?\)$'),
    ...
    def _is_block_start(self, line: str) -> bool:
        ...
        return bool(
            self.patterns['function_def'].match(line) or
            self.patterns['if_statement'].match(line) or
            self.patterns['else_statement'].match(line) or
            self.patterns['for_loop'].match(line) or
            self.patterns['while_loop'].match(line)
        )
...
class SyntaxTreeVisitor:
    ...
    def visit_goal(self, node: SyntaxNode) -> str:
        """Visit a goal node"""
        return f"Goal: {node.value} (Priority: {node.metadata.get('priority', 'medium')})"
