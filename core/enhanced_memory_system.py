...
class VectorMemorySystem:
    ...
    def store_semantic_memory(self, concept: str, knowledge: Dict[str, Any], importance: float = 0.7):
    ...
    def recall_by_importance(self, threshold: float = 0.7, limit: int = 20) -> List[Dict]:
        ...
        important_memories.sort(key=lambda x: (x.get("importance", 0), x["timestamp"]), reverse=True)
    ...
    def forget_low_importance_memories(self, threshold: float = 0.2, max_age_days: int = 30):
        ...
        self.episodic_memories = [
            ...
            if not (memory["importance"] < threshold and 
                   datetime.fromisoformat(memory["timestamp"]) < cutoff_date)
        ]
        ...
        for concept, memory in list(self.semantic_memories.items()):
            if (memory["importance"] < threshold and 
                datetime.fromisoformat(memory["timestamp"]) < cutoff_date and
                memory.get("confidence", 1.0) < 0.5):
    ...
    def get_memory_statistics(self) -> Dict[str, Any]:
        ...
        for memory in self.episodic_memories + list(self.semantic_memories.values()) + list(self.procedural_memories.values()):
    ...
    def _update_memory_index(self, memory_id: str, memory_type: str, content: str, importance: float):
