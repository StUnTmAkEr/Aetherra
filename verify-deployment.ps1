# NeuroCode Domain Verification Script (PowerShell)
# Tests DNS propagation and GitHub Pages deployment

Write-Host "=== NeuroCode Domain Verification ===" -ForegroundColor Cyan
Write-Host ""

# Test 1: DNS Resolution
Write-Host "1. Testing DNS resolution for httpsneurocode.dev..." -ForegroundColor Yellow
try {
    $dns = Resolve-DnsName httpsneurocode.dev -Type A -ErrorAction Stop
...
Write-Host "2. Testing CNAME resolution for www.httpsneurocode.dev..." -ForegroundColor Yellow
try {
    $cname = Resolve-DnsName www.httpsneurocode.dev -Type CNAME -ErrorAction Stop
...
Write-Host "3. Testing HTTP response from httpsneurocode.dev..." -ForegroundColor Yellow
try {
    $response = Invoke-WebRequest -Uri "https://httpsneurocode.dev" -Method Head -UseBasicParsing -ErrorAction Stop
...
try {
    $content = Invoke-WebRequest -Uri "https://httpsneurocode.dev" -UseBasicParsing -ErrorAction Stop
    if ($content.Content -match "NeuroCode") {
        Write-Host "✓ GitHub Pages serving NeuroCode content" -ForegroundColor Green
    } else {
        Write-Host "⚠ GitHub Pages serving content, but NeuroCode branding not detected" -ForegroundColor Orange
    }
...
try {
    $cnameFile = Invoke-WebRequest -Uri "https://raw.githubusercontent.com/Zyonic88/NeuroCode/main/CNAME" -UseBasicParsing -ErrorAction Stop
    $cnameContent = $cnameFile.Content.Trim()
    if ($cnameContent -eq "httpsneurocode.dev") {
