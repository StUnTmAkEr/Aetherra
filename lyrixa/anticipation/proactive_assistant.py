...
class ProactiveAssistant:
    ...
    def _convert_activities_to_context(
    ...
    async def _update_active_suggestions(
        self, new_suggestions: List[Dict[str, Any]]
    ) -> None:
        ...
        if len(self.active_suggestions) > self.max_active_suggestions:
            ...
            self.active_suggestions.sort(
                key=lambda x: x.get("final_score", x.get("confidence", 0)), reverse=True
            )
            self.active_suggestions = self.active_suggestions[
                : self.max_active_suggestions
            ]

    def _is_suggestion_still_relevant(
        self, suggestion: Dict[str, Any], current_time: datetime
    ) -> bool:
        ...
        if self.current_context:
            if (
                suggestion_category == "wellbeing"
                and self.current_context.focus_level < 0.5
            ):
    ...
    def _is_duplicate_suggestion(self, new_suggestion: Dict[str, Any]) -> bool:
        ...
        for existing in self.active_suggestions:
            if (
                existing.get("category") == new_category
    ...
    async def handle_user_response(
        self, suggestion_id: str, response: str, feedback_score: Optional[float] = None
    ) -> None:
        ...
        try:
            for minute in range(duration_minutes):
                ...
                if minute % 15 == 0:  # Every 15 minutes, change activity
                    activity_type = ["coding", "research", "writing", "break"][
                        minute // 15 % 4
                    ]
...
async def test_proactive_assistant():
    ...
    suggestions = assistant.suggestion_generator.generate_suggestions(
        context=context_dict, activity_history=mock_activities, max_suggestions=3
    )
