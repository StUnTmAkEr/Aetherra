...
class ContextAnalyzer:
    ...
    def analyze_context(
        ...
        transition_probability = self._calculate_transition_probability(recent_activities)
    ...
    def _calculate_focus_level(self, activities: List[Dict[str, Any]]) -> float:
        ...
        if len(activities) > 0:
            avg_duration = sum(a.get("duration", 0) for a in activities) / len(activities)
            focus_indicators["single_task_duration"] = min(1.0, avg_duration / 3600)  # Normalize to 1 hour
        ...
        if total_activities > 0:
            focus_indicators["activity_consistency"] = 1.0 - (unique_activities / total_activities)
        ...
        # Weighted average
        weights = {"single_task_duration": 0.4, "activity_consistency": 0.3, "intensity_level": 0.3}
    ...
    def _calculate_productivity_score(self, activities: List[Dict[str, Any]]) -> float:
        ...
        productive_activities = {"coding", "writing", "designing", "researching", "planning"}
        ...
        break_penalty = max(0, break_count / len(activities) - 0.3)  # Penalty if >30% breaks
        
        productivity_score = (activity_ratio * 0.6 + duration_ratio * 0.4) - break_penalty
    ...
    def _calculate_time_in_state(
        ...
        for activity in reversed(activities):
            if activity.get("activity_type", "") == current_activity:
                ...
                if timestamp_str:
                    try:
                        state_start = datetime.fromisoformat(timestamp_str.replace('Z', '+00:00'))
    ...
    def _calculate_transition_probability(self, activities: List[Dict[str, Any]]) -> float:
        ...
        for i in range(1, len(activities)):
            if activities[i].get("activity_type") != activities[i-1].get("activity_type"):
    ...
    def _generate_context_actions(
        ...
        if focus_level < 0.3:
            ...
            actions.extend([
                ...
                "Take short breaks to sustain attention"
            ])
    ...
    def _get_default_context(self) -> ContextInfo:
        ...
        return ContextInfo(
            ...
            suggested_actions=["Start a productive activity", "Set clear goals", "Choose your priority task"]
        )
    ...
    def get_context_trends(self, hours: int = 24) -> Dict[str, Any]:
        ...
        recent_history = self.analysis_history[-hours:] if len(self.analysis_history) > hours else self.analysis_history
        
        # Calculate averages
        avg_focus = sum(c.focus_level for c in recent_history) / len(recent_history)
        avg_productivity = sum(c.productivity_score for c in recent_history) / len(recent_history)
