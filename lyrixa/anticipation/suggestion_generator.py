...
class SuggestionGenerator:
    ...
    def _generate_from_template(
        ...
        title = template.title_template.format(activity=activity, duration=duration)

        description = template.description_template.format(
            activity=activity, duration=duration, current_activity=activity
        )
    ...
    def _check_triggers(
        ...
        for trigger in triggers:
            if trigger == "long_session" and time_in_state > 3600:  # 1+ hours
                ...
                trigger == "high_focus_extended"
                and focus_level > 0.8
                and time_in_state > 1800
            ):  # 30+ min
                return True
                ...
                if len(activity_history) >= 3:
                    recent_durations = [
                        a.get("duration", 0) for a in activity_history[-3:]
                    ]
                ...
                if len(activity_history) >= 3:
                    recent_types = [
                        a.get("activity_type", "") for a in activity_history[-3:]
                    ]
                    if len(set(recent_types)) == len(recent_types):  # All different
                        return True
            elif trigger == "overwork_detected":
                total_work_time = sum(
                    a.get("duration", 0) for a in activity_history[-10:]
                )
    ...
    def _generate_pattern_suggestions(
        self, activity_history: List[Dict[str, Any]], context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for sequence, frequency in sequences.items():
            if frequency >= 2:  # Pattern occurred at least twice
                suggestion = {
                    ...
                    "actions": [
                        f"Proceed with {sequence[1]}",
                        "Review workflow efficiency",
                    ],
                    "confidence": 0.6
                    + (frequency * 0.1),  # Higher confidence for frequent patterns
                    "source": "pattern",
                    "created_at": datetime.now().isoformat(),
                }
                suggestions.append(suggestion)
    ...
    def _generate_time_based_suggestions(
        self, context: Dict[str, Any], activity_history: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _calculate_context_bonus(
        self, template: SuggestionTemplate, context: Dict[str, Any]
    ) -> float:
        ...
        if (
            template.category == SuggestionCategory.PRODUCTIVITY
            and productivity_score < 0.5
        ):
            bonus += 0.1
        ...
        if (
            template.category == SuggestionCategory.OPTIMIZATION
            and productivity_score < 0.6
        ):
            bonus += 0.1
    ...
    def _calculate_recency_penalty(self, suggestion: Dict[str, Any]) -> float:
        ...
        recent_suggestions = [
            s
            for s in self.suggestion_history[-10:]  # Last 10 suggestions
    ...
    def update_user_preferences(self, category: str, feedback: float) -> None:
