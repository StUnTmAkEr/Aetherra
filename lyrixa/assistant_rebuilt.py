...
""")
    ...
    async def generate_aether_code(
        ...
        try:
            ...
            generated_code = await self.aether_interpreter.generate_from_description(
                description, similar_patterns, context
            )
            ...
            return {
                ...
                "suggestions": await self._generate_code_improvements(generated_code),
            }
...
async def create_lyrixa(
    workspace_path: Optional[str] = None, config: Optional[Dict[str, Any]] = None
) -> LyrixaAI:
    """Create and initialize a Lyrixa AI instance"""
    lyrixa = LyrixaAI(workspace_path, config)
...
async def quick_chat(message: str, workspace_path: Optional[str] = None) -> str:
    ...
    lyrixa = await create_lyrixa(workspace_path)
    response = await lyrixa.chat(message)
    await lyrixa.cleanup()
    return response.get("text", "I couldn't process that request.")
