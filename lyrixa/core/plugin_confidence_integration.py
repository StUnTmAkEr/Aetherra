...
class ConfidenceEnhancedPluginSystem:
    ...
    def analyze_plugin_safely(self, plugin_name: str, plugin_path: Path) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            analysis = self.confidence_scorer.analyze_plugin(plugin_name, plugin_code, plugin_path)
    ...
    def load_plugin_with_confidence_check(self, plugin_name: str, plugin_path: Path,
        ...
        should_block = should_block_plugin_execution(risk_level, confidence_score, user_override)
        ...
        warning = get_plugin_confidence_warning(plugin_name, confidence_score, risk_level)
    ...
    def execute_plugin_with_monitoring(self, plugin_name: str, command: str, 
                                     **kwargs) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            current_confidence = self.confidence_scorer.get_plugin_confidence(plugin_name)
    ...
    def get_confidence_dashboard_data(self) -> Dict[str, Any]:
        ...
        medium_confidence = len([p for p in all_scores if 0.5 <= p['confidence_score'] < 0.8])
        low_confidence = len([p for p in all_scores if p['confidence_score'] < 0.5])
        blocked_plugins = len([p for p in all_scores if p['risk_level'] in ['HIGH', 'CRITICAL']])
    ...
    def suggest_plugin_alternatives(self, plugin_name: str, required_capabilities: Optional[List[str]] = None) -> List[Dict]:
        """Suggest alternative plugins with higher confidence scores."""
        
        all_scores = self.confidence_scorer.get_all_plugin_scores()
        current_plugin = next((p for p in all_scores if p['plugin_name'] == plugin_name), None)
...
class LyrixaConfidenceIntegration:
    ...
    def __init__(self, lyrixa_instance):
        self.lyrixa = lyrixa_instance
        self.enhanced_system = ConfidenceEnhancedPluginSystem(lyrixa_instance=lyrixa_instance)
    ...
    def handle_confidence_warning(self, plugin_name: str, warning: str, analysis: Dict):
        ...
        if hasattr(self.lyrixa, 'memory_system'):
            self.lyrixa.memory_system.store_plugin_warning(plugin_name, warning, analysis)
    
    def handle_plugin_block(self, plugin_name: str, risk_level: str, confidence_score: float, analysis: Dict):
    ...
    def handle_recommendations(self, plugin_name: str, recommendations: List[Dict]):
        ...
        if hasattr(self.lyrixa, 'show_recommendations'):
            ...
            logger.info(f"RECOMMENDATIONS for {plugin_name}: {len(recommendations)} suggestions available")
...
'''
    ...
    try:
        ...
        print(f"Risk Level: {result.get('safety_analysis', {}).get('risk_level', 'UNKNOWN')}")
    finally:
