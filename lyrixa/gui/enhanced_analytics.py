...
try:
    ...
    from PySide6.QtGui import QColor, QFont, QPalette, QPainter, QLinearGradient, QBrush
    from PySide6.QtWidgets import (
        ...
        QTreeWidget, QTreeWidgetItem, QSpinBox, QDial
    )
...
class RealTimeProductivityWidget(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def __init__(self, parent=None):
        ...
        self.metrics = ProductivityMetrics(
            focus_score=75.0, efficiency_rating=7.5, task_completion_rate=0.8,
            goal_progress=0.65, break_balance=0.7, mood_stability=0.8,
            energy_level=0.75, creativity_index=0.6
        )
    ...
    def init_ui(self):
        ...
        self.focus_gauge = self.create_gauge("Focus Score", self.metrics.focus_score, "#3498db")
        self.efficiency_gauge = self.create_gauge("Efficiency", self.metrics.efficiency_rating * 10, "#e74c3c")
        self.progress_gauge = self.create_gauge("Goal Progress", self.metrics.goal_progress * 100, "#27ae60")
        self.mood_gauge = self.create_gauge("Mood Stability", self.metrics.mood_stability * 100, "#f39c12")
...
class AgentUsageAnalyzer(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def __init__(self, parent=None):
        ...
        self.usage_metrics = AgentUsageMetrics(
            total_interactions=156, successful_assists=142, response_time_avg=1.2,
            user_satisfaction=4.3, feature_usage={
                "memory_search": 45, "code_analysis": 32, "goal_tracking": 28,
                "suggestions": 51, "documentation": 15
            }, error_rate=0.09, learning_progress=0.74
        )
    ...
    def init_ui(self):
        ...
        # Success rate
        success_rate = (self.usage_metrics.successful_assists / 
                       self.usage_metrics.total_interactions * 100)
        success_widget = self.create_metric_card("Success Rate", f"{success_rate:.1f}%", "#27ae60")
        overview_layout.addWidget(success_widget)
        
        # Response time
        response_widget = self.create_metric_card("Avg Response", f"{self.usage_metrics.response_time_avg:.1f}s", "#3498db")
        overview_layout.addWidget(response_widget)
        
        # User satisfaction
        satisfaction_widget = self.create_metric_card("Satisfaction", f"{self.usage_metrics.user_satisfaction:.1f}/5", "#f39c12")
        overview_layout.addWidget(satisfaction_widget)
        
        # Learning progress
        learning_widget = self.create_metric_card("Learning", f"{self.usage_metrics.learning_progress*100:.0f}%", "#9b59b6")
        ...
        for feature, count in sorted(self.usage_metrics.feature_usage.items(), 
    ...
    def create_feature_bar(self, feature: str, count: int, percentage: float) -> QWidget:
    ...
    def generate_recommendations(self) -> str:
        ...
        if self.usage_metrics.error_rate > 0.1:
            recommendations.append("• High error rate detected. Consider reviewing common failure patterns.")
            
        if self.usage_metrics.response_time_avg > 2.0:
            recommendations.append("• Response times are elevated. Check system performance.")
            
        if self.usage_metrics.user_satisfaction < 4.0:
            recommendations.append("• User satisfaction could be improved. Review feedback patterns.")
        ...
        recommendations.append(f"• Most used feature: {max_feature[0]} ({max_feature[1]} uses)")
        recommendations.append(f"• Least used feature: {min_feature[0]} ({min_feature[1]} uses)")
        recommendations.append("• Consider promoting underutilized features or improving their discoverability.")
