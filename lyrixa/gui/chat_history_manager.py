...
class ChatHistoryManager:
    ...
    def add_message(self, sender: str, content: str, message_type: str = "text", 
                   context_data: Dict = None, thread_id: str = None) -> str:
        """Add a message to the current session."""
        message_hash = hashlib.md5(f"{sender}:{content}:{datetime.now().isoformat()}".encode()).hexdigest()
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (self.current_session_id, sender, message_type, content, 
              json.dumps(context_data or {}), message_hash, thread_id))
    ...
    def _extract_topics(self, content: str):
        ...
        topic_keywords = {
            "plugin_development": ["plugin", "create plugin", "plugin system", "extension"],
        ...
        for topic, keywords in topic_keywords.items():
            confidence = sum(1 for keyword in keywords if keyword in content_lower) / len(keywords)
            if confidence > 0:
        ...
        for topic, confidence in detected_topics:
            ...
            """, (self.current_session_id, topic, confidence, 
                  self.current_session_id, topic))
    ...
    def get_conversation_summary(self, session_id: str = None) -> Dict:
        ...
        return {
            ...
            "topics": [{"topic": t[0], "confidence": t[1], "mentions": t[2]} for t in topics],
    ...
    def export_session(self, session_id: str = None, format: str = "json") -> str:
        ...
        if format == "json":
            ...
            lines = [f"Chat Session: {session_id}", f"Started: {summary.get('start_time', 'Unknown')}", "=" * 50, ""]
            for msg in messages:
