...
class PersonalityManager:
        ...
        try:
            if self.config_path.exists():
                with open(self.config_path, 'r') as f:
                    config = json.load(f)
                    self.current_personality = config.get('current_personality', 'balanced')
    ...
    def get_current_personality(self) -> Dict[str, Any]:
        ...
        return self.personalities.get(self.current_personality, self.personalities["balanced"])
    ...
    def create_custom_personality(self, name: str, description: str, traits: Dict[str, float], 
    ...
    def record_user_feedback(self, response_id: str, feedback_type: str, rating: int):
        ...
        if len(self.user_feedback) > 1000:
            oldest_keys = sorted(self.user_feedback.keys())[:len(self.user_feedback) - 1000]
    ...
    def get_personality_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "trait_summary": {
                "most_dominant": max(traits.items(), key=lambda x: x[1]) if traits else ("none", 0),
                "least_dominant": min(traits.items(), key=lambda x: x[1]) if traits else ("none", 0),
            ...
            "custom_adjustments": len(self.custom_traits.get(self.current_personality, {}))
        }
    ...
    def import_personality(self, personality_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if name and "traits" in personality_data and "response_style" in personality_data:
