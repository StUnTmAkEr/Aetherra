...
class FeedbackCollector:
    ...
    def collect_thumbs_feedback(self, 
                               is_positive: bool, 
    ...
    def collect_edit_feedback(self, 
        ...
        feedback = UserFeedback(
            id=str(uuid.uuid4()),
            feedback_type=feedback_type_map.get(item_type, FeedbackType.SUGGESTION_EDIT),
            timestamp=datetime.now(),
            context=context or self.current_context.copy(),
    ...
    def get_feedback_summary(self, days: int = 7) -> Dict[str, Any]:
        ...
        # Calculate metrics
        positive_feedback = len([f for f in relevant_feedback 
                               if f.feedback_type == FeedbackType.THUMBS_UP])
        negative_feedback = len([f for f in relevant_feedback 
        ...
        for f in relevant_feedback:
            feedback_by_type[f.feedback_type.value] = feedback_by_type.get(f.feedback_type.value, 0) + 1
    ...
    def _calculate_satisfaction_trend(self, feedback_list: List[UserFeedback]) -> str:
        ...
        first_avg = statistics.mean([f.rating for f in first_half if f.rating is not None])
        second_avg = statistics.mean([f.rating for f in second_half if f.rating is not None])
...
class AdaptiveLearningEngine:
    ...
    def _analyze_immediate_adaptations(self, feedback: UserFeedback) -> List[Dict[str, Any]]:
        """Analyze feedback for immediate adaptations."""
        adaptations = []
        
        # Strong negative feedback triggers immediate intervention reduction
        if (feedback.feedback_type == FeedbackType.THUMBS_DOWN and 
            feedback.rating and feedback.rating <= 2.0):
        ...
        if feedback.feedback_type == FeedbackType.PREFERENCE_CHANGE:
            adaptations.append({
                "dimension": AdaptationDimension.INTERVENTION_FREQUENCY,
                "direction": feedback.rating / 5.0 - 0.5,  # Convert rating to adaptation
    ...
    def _analyze_timing_patterns(self, feedback_list: List[UserFeedback]) -> List[Dict[str, Any]]:
        ...
        for hour, feedback_group in hourly_feedback.items():
            if len(feedback_group) >= 3:  # Minimum sample size
                negative_ratio = len([f for f in feedback_group 
                                    if f.feedback_type == FeedbackType.THUMBS_DOWN]) / len(feedback_group)
    ...
    def _analyze_style_patterns(self, feedback_list: List[UserFeedback]) -> List[Dict[str, Any]]:
        """Analyze user style preferences from feedback."""
        adaptations = []
        
        edit_feedback = [f for f in feedback_list if f.feedback_type == FeedbackType.SUGGESTION_EDIT]
        ...
        if len(edit_feedback) >= 3:
            ...
            for feedback in edit_feedback:
                if feedback.original_content and feedback.edited_content:
                    ratio = len(feedback.edited_content) / len(feedback.original_content)
    ...
    def _analyze_frequency_patterns(self, feedback_list: List[UserFeedback]) -> List[Dict[str, Any]]:
        """Analyze intervention frequency preferences."""
        adaptations = []
        
        # Count negative reactions to suggestions
        suggestion_feedback = [f for f in feedback_list 
                             if f.suggestion_id is not None]
        
        if len(suggestion_feedback) >= 5:
            negative_ratio = len([f for f in suggestion_feedback 
                                if f.feedback_type == FeedbackType.THUMBS_DOWN]) / len(suggestion_feedback)
    ...
    def _apply_adaptation(self, adaptation: Dict[str, Any]):
        ...
        if adaptation["confidence"] < self.adaptation_thresholds["confidence_threshold"]:
        ...
        if dimension == AdaptationDimension.INTERVENTION_FREQUENCY:
            self.personality_profile.intervention_frequency = max(0.0, min(1.0, 
                self.personality_profile.intervention_frequency + direction))
                
        elif dimension == AdaptationDimension.DETAIL_LEVEL:
            self.personality_profile.detail_preference = max(0.0, min(1.0,
                self.personality_profile.detail_preference + direction))
                
        elif dimension == AdaptationDimension.PROACTIVITY:
            self.personality_profile.proactivity_preference = max(0.0, min(1.0,
                self.personality_profile.proactivity_preference + direction))
                
        elif dimension == AdaptationDimension.FORMALITY_LEVEL:
            self.personality_profile.formality_preference = max(0.0, min(1.0,
                self.personality_profile.formality_preference + direction))
        ...
        logger.info(f"Applied adaptation: {dimension.value} {direction:+.3f} ({adaptation['reason']})")
...
class LiveFeedbackInterface:
    ...
    def present_suggestion_with_feedback(self, 
                                       suggestion_id: str,
                                       suggestion_text: str,
    ...
    def handle_feedback_action(self, 
        ...
        try:
            if action_type == "positive_feedback":
                feedback = self.learning_engine.feedback_collector.collect_thumbs_feedback(
                    True, suggestion_id=item_id
                )
                self.learning_engine.process_feedback(feedback)
                
                return {"status": "success", "message": "Thanks for the positive feedback!"}
                ...
                return {"status": "success", "message": "Thanks for the feedback. I'll adjust future suggestions."}
                ...
                if item_id in self.active_suggestions:
                    ...
                    feedback = self.learning_engine.feedback_collector.collect_edit_feedback(
                    ...
                    return {"status": "success", "message": "Suggestion updated. I'm learning from your edits!"}
                    
            elif action_type == "rate_interaction":
                rating = data.get("rating", 3.0)
                comment = data.get("comment", "")
                
                feedback = self.learning_engine.feedback_collector.collect_rating_feedback(
                    rating, FeedbackType.INTERACTION_QUALITY, comment
                )
                self.learning_engine.process_feedback(feedback)
                
                return {"status": "success", "message": f"Thanks for rating our interaction {rating}/5!"}
    ...
    def get_learning_insights(self) -> Dict[str, Any]:
        ...
        feedback_summary = self.learning_engine.feedback_collector.get_feedback_summary()
        ...
        return {
            ...
            "learning_stats": {
                "total_feedback_items": len(self.learning_engine.feedback_collector.feedback_history),
                "adaptations_made": len(self.learning_engine.adaptation_history),
                "learning_rate": self.learning_engine.learning_rate,
                "current_satisfaction_trend": feedback_summary.get("satisfaction_trend", "unknown")
            },
...
async def demo_feedback_loop():
    ...
    actions = [
        ...
        ("edit_suggestion", "sug_001", {"edited_text": "Take a 5-minute stretch break."}),
        ("rate_interaction", "", {"rating": 4.0, "comment": "Helpful but could be more specific"})
    ]
