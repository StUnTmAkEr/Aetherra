...
class ContextMemoryManager:
    ...
    def _record_context_switch(
        self, old_context: str, new_context: str, metadata: Dict
    ):
    ...
    def get_context_patterns(self, context: str) -> List[Dict]:
        """Get patterns for a specific context type."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        for row in cursor.fetchall():
            try:
                ...
                patterns.append(
                    {"pattern": pattern_data, "frequency": row[1], "last_seen": row[2]}
                )
    ...
    def get_user_preferences(self, context: str | None = None) -> Dict:
        """Get user preferences for a specific context or all contexts."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        if context:
            cursor.execute(
                """
                SELECT preferred_response_style, common_requests, assistance_level
            ...
            if result:
                preferences = {
                    "response_style": result[0],
                    "common_requests": result[1],
                    "assistance_level": result[2],
                }
            else:
                preferences = {}
        else:
            cursor.execute("SELECT * FROM user_context_preferences")
            preferences = {}
            for row in cursor.fetchall():
                preferences[row[0]] = {
                    "response_style": row[1],
                    "common_requests": row[2],
                    "assistance_level": row[3],
                }

        conn.close()
        return preferences

    def update_context_duration(self, context: str, duration_seconds: int):
        """Update the duration for the current context session."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        cursor.execute(
            ...
            WHERE context_type = ? AND timestamp = (
                SELECT MAX(timestamp) FROM context_sessions WHERE context_type = ?
            )
        """,
            (duration_seconds, context, context),
        )
