...
try:
    from PySide6.QtWidgets import (
        ...
        QTableWidget, QTableWidgetItem, QTextEdit, QScrollArea, QFrame,
        QGridLayout, QProgressBar, QComboBox, QDateEdit, QGroupBox,
...
class MetricsCalculator:
    ...
    def calculate_productivity_metrics(self, activities: List[Dict]) -> Dict[str, Any]:
        ...
        productivity_score = min(100, (focus_time / (focus_time + break_time)) * 100) if (focus_time + break_time) > 0 else 0
    ...
    def calculate_suggestion_metrics(self, suggestions: List[Dict]) -> Dict[str, Any]:
        ...
        accepted_suggestions = sum(1 for sug in suggestions if sug.get('accepted', False))
...
class ChartWidget(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def __init__(self, parent=None):
        ...
        self.canvas = FigureCanvas(self.figure) if MATPLOTLIB_AVAILABLE and self.figure else None
    ...
    def plot_productivity_trend(self, data: List[Dict]):
        ...
        dates = [datetime.fromisoformat(item['date']) if isinstance(item.get('date'), str) else item.get('date', datetime.now()) for item in data]
    ...
    def plot_suggestion_effectiveness(self, categories: Dict[str, Dict[str, int]]):
        ...
        if not categories:
            ax.text(0.5, 0.5, 'No suggestion data', ha='center', va='center', transform=ax.transAxes)
            self.canvas.draw()
            return
        ...
        ax.bar(x - width/2, accepted, width, label='Accepted', color='green', alpha=0.7)
        ax.bar(x + width/2, rejected, width, label='Rejected', color='red', alpha=0.7)
        
        ax.set_title('Suggestion Effectiveness by Category', fontsize=14, fontweight='bold')
...
class AnalyticsDashboard(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def __init__(self, parent=None):
        if not PYSIDE6_AVAILABLE:
            logger.warning("PySide6 not available. Analytics dashboard will not function.")
            return
    ...
    def update_metrics(self):
        ...
        self.productivity_labels['sessions'].setText(f"Sessions: {prod_metrics['total_sessions']}")
        self.productivity_labels['avg_length'].setText(f"Avg Length: {prod_metrics['avg_session_length']:.1f} min")
        self.productivity_labels['score'].setText(f"Score: {prod_metrics['productivity_score']:.1f}%")
        self.productivity_labels['focus_time'].setText(f"Focus Time: {prod_metrics['focus_time']:.1f} min")
        self.productivity_labels['efficiency'].setText(f"Efficiency: {prod_metrics['efficiency_rating']:.1f}/10")
        ...
        self.pattern_labels['total'].setText(f"Total Patterns: {pattern_metrics['total_patterns']}")
        self.pattern_labels['common'].setText(f"Most Common: {pattern_metrics['most_common_pattern']}")
        self.pattern_labels['confidence'].setText(f"Avg Confidence: {pattern_metrics['pattern_confidence']:.1f}%")
        self.pattern_labels['recurring'].setText(f"Recurring: {pattern_metrics['recurring_patterns']}")
        ...
        self.suggestion_labels['total'].setText(f"Total Suggestions: {suggestion_metrics['total_suggestions']}")
        self.suggestion_labels['accepted'].setText(f"Accepted: {suggestion_metrics['accepted_suggestions']}")
        self.suggestion_labels['rate'].setText(f"Acceptance Rate: {suggestion_metrics['acceptance_rate']:.1f}%")
        self.suggestion_labels['confidence'].setText(f"Avg Confidence: {suggestion_metrics['avg_confidence']:.1f}%")
        self.suggestion_labels['top_category'].setText(f"Top Category: {suggestion_metrics['top_category']}")
    ...
    def export_data(self):
        ...
        try:
            ...
            filename = f"lyrixa_analytics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            QMessageBox.information(self, "Export", f"Analytics data exported to {filename}")
