...
class ResponseStyleMemoryManager:
    ...
    def record_feedback(
        ...
        try:
            ...
            self._process_feedback_for_adaptation(
                context_type, feedback_type, user_rating
            )
            return True
    ...
    def _process_feedback_for_adaptation(
        self, context_type: str, feedback_type: str, rating: int
    ):
        ...
        adaptation_value = self.adaptation_rules.get(feedback_type, {}).get(
            "default", 0.0
        )
        ...
        try:
            ...
            if result:
                ...
                new_value = (current_value * sample_count + adaptation_value) / (
                    sample_count + 1
                )
    ...
    def learn_from_conversation(self, conversation_data: List[Dict[str, Any]]):
        ...
        user_messages = [
            msg for msg in conversation_data if msg.get("sender") == "user"
        ]

        # Extract patterns
        patterns = {
            "average_user_message_length": statistics.mean(
    ...
    def _calculate_engagement(self, user_messages: List[Dict[str, Any]]) -> float:
        ...
        score += min(
            1.0, engagement_indicators["total_length"] / (len(user_messages) * 50)
        )
    ...
    def _analyze_response_timing(
        ...
        return {
            "average_response_gap": statistics.mean(response_gaps)
            if response_gaps
            else 0,
            "average_user_response": statistics.mean(user_response_times)
    ...
    def _store_conversation_patterns(self, patterns: Dict[str, Any]):
        ...
        try:
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                ("conversation_pattern", json.dumps(patterns), "general", 1.0),
            )
    ...
    def reset_learning_data(self, confirm: bool = False):
        ...
        if not confirm:
            print(
                "Warning: This will delete all learned preferences. Call with confirm=True to proceed."
            )
            return False
