...
class PluginConfidenceWidget(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        self.name_label = ttk.Label(
            main_frame, text=self.plugin_name, font=("Arial", 10, "bold")
        )
        ...
        self.details_button = ttk.Button(
            main_frame, text="Details", command=self.show_details
        )
    ...
    def show_details(self):
        ...
        if hasattr(self, "analysis") and self.analysis:
            ...
            messagebox.showinfo(
                "Details", f"No detailed analysis available for {self.plugin_name}"
            )
...
class ConfidenceDetailsDialog:
    ...
    def create_dialog(self):
        ...
        close_button = ttk.Button(
            self.dialog, text="Close", command=self.dialog.destroy
        )
    ...
    def create_overview_tab(self, notebook):
        ...
        # Confidence score
        ttk.Label(
            frame, text="Overall Confidence Score:", font=("Arial", 12, "bold")
        ).pack(anchor="w", pady=5)
        ttk.Label(
            frame, text=f"{self.analysis['confidence_score']:.1%}", font=("Arial", 14)
        ).pack(anchor="w", padx=20)

        # Risk level
        ttk.Label(frame, text="Risk Level:", font=("Arial", 12, "bold")).pack(
            anchor="w", pady=5
        )
        ...
        # Safety score
        ttk.Label(frame, text="Safety Score:", font=("Arial", 12, "bold")).pack(
            anchor="w", pady=5
        )
        safety_score = self.analysis["safety_analysis"]["safety_score"]
        ttk.Label(frame, text=f"{safety_score:.1f}/100", font=("Arial", 14)).pack(
            anchor="w", padx=20
        )

        # Timestamp
        ttk.Label(frame, text="Analysis Time:", font=("Arial", 12, "bold")).pack(
            anchor="w", pady=5
        )
        ttk.Label(frame, text=self.analysis["timestamp"], font=("Arial", 10)).pack(
            anchor="w", padx=20
        )
    ...
    def create_safety_tab(self, notebook):
        ...
        if safety["issues"]:
            ...
            for i, issue in enumerate(safety["issues"], 1):
                text_area.insert(
                    "end", f"{i}. {issue['type']} ({issue['severity']})\\n"
                )
        ...
        if safety["warnings"]:
            ...
            for i, warning in enumerate(safety["warnings"], 1):
                text_area.insert(
                    "end", f"{i}. {warning['type']} ({warning['severity']})\\n"
                )
    ...
    def create_runtime_tab(self, notebook):
        ...
        if metrics:
            ttk.Label(
                frame,
                text=f"Total Executions: {metrics.get('total_executions', 0)}",
                font=("Arial", 10),
            ).pack(anchor="w", pady=2)
            ttk.Label(
                frame,
                text=f"Success Rate: {metrics.get('success_rate', 0):.1%}",
                font=("Arial", 10),
            ).pack(anchor="w", pady=2)
            ttk.Label(
                frame,
                text=f"Average Execution Time: {metrics.get('avg_execution_time', 0):.3f}s",
                font=("Arial", 10),
            ).pack(anchor="w", pady=2)
            ttk.Label(
                frame,
                text=f"Error Frequency: {metrics.get('error_frequency', 0):.1%}",
                font=("Arial", 10),
            ).pack(anchor="w", pady=2)
        else:
            ttk.Label(
                frame, text="No runtime metrics available", font=("Arial", 10)
    ...
    def create_recommendations_tab(self, notebook):
        ...
        if recommendations:
            for i, rec in enumerate(recommendations, 1):
                ttk.Label(
                    frame, text=f"{i}. {rec['type']}", font=("Arial", 10, "bold")
                ).pack(anchor="w", pady=2)
                ttk.Label(frame, text=f"   {rec['message']}", font=("Arial", 10)).pack(
                    anchor="w", padx=20
                )
        else:
            ttk.Label(
                frame, text="No recommendations available", font=("Arial", 10)
            ).pack(anchor="w", pady=20)
...
class PluginConfidenceDashboard(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            self, text="Plugin Confidence Dashboard", font=("Arial", 16, "bold")
        )
        ...
        ttk.Button(toolbar, text="Refresh All", command=self.refresh_all).pack(
            side="left", padx=5
        )
...
def main():
    ...
    # Add some demo data
    demo_plugins = [
        {"name": "Text Processor", "confidence": 0.95, "risk": "LOW", "safety": 98.0},
        {"name": "File Manager", "confidence": 0.65, "risk": "MEDIUM", "safety": 75.0},
        {"name": "Network Client", "confidence": 0.35, "risk": "HIGH", "safety": 40.0},
    ]
