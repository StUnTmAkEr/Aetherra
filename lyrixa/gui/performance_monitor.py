...
try:
    from PySide6.QtWidgets import (
        ...
        QGridLayout, QFrame, QScrollArea, QSplitter, QTextEdit
    )
    ...
    def collect_metrics(self):
        ...
        try:
            ...
            if self.network_history:
                ...
                if time_diff > 0:
                    bytes_sent_per_sec = (network.bytes_sent - prev_network.bytes_sent) / time_diff
                    bytes_recv_per_sec = (network.bytes_recv - prev_network.bytes_recv) / time_diff
                else:
...
class MetricsTable(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def update_metrics(self, metrics_data: Dict[str, Any]):
        ...
        display_metrics = [
            ...
            ("Network Receive", f"{metrics_data.get('network_recv', 0) / 1024:.1f} KB/s"),
        ]
...
class PerformanceMonitor(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def __init__(self, parent=None):
        if not PYSIDE6_AVAILABLE:
            logger.warning("PySide6 not available. Performance monitor will not function.")
            return
