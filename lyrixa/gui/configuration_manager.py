...
try:
    from PySide6.QtWidgets import (
        ...
        QGroupBox, QFormLayout, QLineEdit, QSpinBox, QDoubleSpinBox, QCheckBox,
        QComboBox, QSlider, QTextEdit, QFileDialog, QMessageBox, QProgressBar,
        QColorDialog, QFontDialog, QScrollArea, QSplitter, QListWidget, QListWidgetItem
    )
    ...
    def run(self):
        ...
        try:
            ...
            backup_data = {
                ...
                'anticipation_settings': asdict(self.config_manager.anticipation_settings),
                'system_configuration': asdict(self.config_manager.system_configuration)
            }
...
class PreferencesTab(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def init_ui(self):
        ...
        self.controls['enable_notifications'].setChecked(self.preferences.enable_notifications)
        notif_form.addRow("Enable Notifications:", self.controls['enable_notifications'])
        ...
        self.controls['share_anonymous_data'].setChecked(self.preferences.share_anonymous_data)
        privacy_form.addRow("Share Anonymous Data:", self.controls['share_anonymous_data'])
    ...
    def update_preferences(self):
        ...
        self.preferences.enable_notifications = self.controls['enable_notifications'].isChecked()
        ...
        self.preferences.enable_telemetry = self.controls['enable_telemetry'].isChecked()
        self.preferences.share_anonymous_data = self.controls['share_anonymous_data'].isChecked()
...
class AnticipationTab(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def init_ui(self):
        ...
        self.controls['pattern_detection_sensitivity'].setValue(self.settings.pattern_detection_sensitivity)
        pattern_form.addRow("Sensitivity:", self.controls['pattern_detection_sensitivity'])
        
        # Minimum occurrences
        self.controls['minimum_pattern_occurrences'] = QSpinBox()
        self.controls['minimum_pattern_occurrences'].setRange(1, 10)
        self.controls['minimum_pattern_occurrences'].setValue(self.settings.minimum_pattern_occurrences)
        pattern_form.addRow("Min Occurrences:", self.controls['minimum_pattern_occurrences'])
        ...
        self.controls['pattern_confidence_threshold'].setValue(self.settings.pattern_confidence_threshold)
        pattern_form.addRow("Confidence Threshold:", self.controls['pattern_confidence_threshold'])
        ...
        for weight_name in ['activity_weight', 'time_weight', 'location_weight']:
            ...
            context_form.addRow(f"{weight_name.replace('_', ' ').title()}:", self.controls[weight_name])
        ...
        self.controls['suggestion_diversity'].setValue(int(self.settings.suggestion_diversity * 100))
        ...
        self.controls['max_concurrent_suggestions'].setValue(self.settings.max_concurrent_suggestions)
        suggestion_form.addRow("Max Concurrent:", self.controls['max_concurrent_suggestions'])
    ...
    def update_settings(self):
        ...
        self.settings.pattern_detection_sensitivity = self.controls['pattern_detection_sensitivity'].value()
        self.settings.minimum_pattern_occurrences = self.controls['minimum_pattern_occurrences'].value()
        self.settings.pattern_confidence_threshold = self.controls['pattern_confidence_threshold'].value()
        ...
        self.settings.suggestion_diversity = self.controls['suggestion_diversity'].value() / 100.0
        self.settings.max_concurrent_suggestions = self.controls['max_concurrent_suggestions'].value()
...
class DataManagementTab(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def export_preferences_only(self):
        ...
        if filename:
            try:
                ...
                QMessageBox.information(self, "Export Successful", f"Preferences exported to:\n{filename}")
                
            except Exception as e:
                QMessageBox.critical(self, "Export Failed", f"Failed to export preferences:\n{e}")
        ...
        if filename:
            try:
                ...
                if 'version' not in import_data:
                    QMessageBox.warning(self, "Import Warning", "Configuration file format not recognized.")
                    return
                ...
                if 'anticipation_settings' in import_data:
                    ...
                    for key, value in settings_data.items():
                        if hasattr(self.config_manager.anticipation_settings, key):
                            setattr(self.config_manager.anticipation_settings, key, value)
    ...
    def create_backup(self):
        ...
        backup_filename = backup_dir / f"lyrixa_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        self.backup_worker = ConfigurationBackupWorker(self.config_manager, str(backup_filename))
    ...
    def on_backup_completed(self, backup_path: str):
        ...
        self.progress_bar.setVisible(False)
        QMessageBox.information(self, "Backup Successful", f"Configuration backed up to:\n{backup_path}")
    ...
    def restore_backup(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            self, "Restore from Backup", str(backup_dir),
            "JSON Files (*.json)"
        )
            ...
            if reply == QMessageBox.Yes:
                try:
                    ...
                    if 'user_preferences' in backup_data:
                        self.config_manager.user_preferences = UserPreferences(**backup_data['user_preferences'])
                    
                    if 'anticipation_settings' in backup_data:
                        self.config_manager.anticipation_settings = AnticipationSettings(**backup_data['anticipation_settings'])
                    
                    if 'system_configuration' in backup_data:
                        self.config_manager.system_configuration = SystemConfiguration(**backup_data['system_configuration'])
                    
                    self.config_manager.save_configuration()
                    QMessageBox.information(self, "Restore Successful", "Configuration restored successfully.")
                    
                except Exception as e:
                    QMessageBox.critical(self, "Restore Failed", f"Failed to restore configuration:\n{e}")
    ...
    def reset_to_defaults(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                QMessageBox.critical(self, "Reset Failed", f"Failed to reset settings:\n{e}")
        ...
        try:
            if self.config_file.exists():
                ...
                if 'anticipation_settings' in config_data:
                    self.anticipation_settings = AnticipationSettings(**config_data['anticipation_settings'])
                
                # Load system configuration
                if 'system_configuration' in config_data:
                    self.system_configuration = SystemConfiguration(**config_data['system_configuration'])
