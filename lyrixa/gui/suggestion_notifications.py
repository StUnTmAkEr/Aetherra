...
try:
    from PySide6.QtWidgets import (
        ...
        QMessageBox, QCheckBox, QSpinBox, QGroupBox, QTextEdit,
...
class SuggestionWidget(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def init_ui(self):
        ...
        priority_label.setStyleSheet(f"color: {priority_color}; font-size: 16px; font-weight: bold;")
...
class SuggestionPanel(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def init_ui(self):
        ...
        self.empty_label.setStyleSheet("color: #6c757d; font-style: italic; margin: 20px;")
    ...
    def add_suggestion(self, suggestion: Suggestion):
        ...
        if len(self.suggestions) >= self.max_suggestions:
            ...
            oldest_id = min(self.suggestions.keys(), key=lambda k: self.suggestions[k].timestamp)
    ...
    def remove_suggestion(self, suggestion_id: str):
        ...
        if suggestion_id in self.suggestions:
            ...
            for i in range(self.suggestions_layout.count()):
                ...
                if isinstance(widget, SuggestionWidget) and widget.suggestion.id == suggestion_id:
...
class SuggestionHistory(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def add_to_history(self, suggestion: Suggestion):
        ...
        # Create list item
        status_icon = "✓" if suggestion.accepted else "✗" if suggestion.accepted is False else "—"
...
class SuggestionNotificationSystem(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def __init__(self, parent=None):
        if not PYSIDE6_AVAILABLE:
            logger.warning("PySide6 not available. Suggestion notification system will not function.")
            return
