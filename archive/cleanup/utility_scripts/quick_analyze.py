            ...
            if pattern.search(line):
                if pattern_name == "unused_import":
                    ...
                    if "print(" in line and (
                        "debug" in line.lower() or "test" in line.lower()
                    ):
                        warnings.append(
                            {
                                "file": str(file_path),
...
def main():
    ...
    if category_counts["unused_import"] > 0:
        print(
            f"  🔧 {category_counts['unused_import']} unused imports can be removed automatically"
        )
    if category_counts["trailing_whitespace"] > 0:
        print(
            f"  🧹 {category_counts['trailing_whitespace']} trailing whitespace issues can be fixed"
        )
