...
try:
    ...
    from lyrixa.core.plugin_confidence_integration import (
        ConfidenceEnhancedPluginSystem
    )
...
class PluginConfidenceTestSuite:
    ...
    def test_safe_plugin(self) -> Dict:
        ...
        if not self.scorer:
            return {"test_name": "Safe Plugin Analysis", "passed": False, "error": "Scorer not available"}
...
'''
        ...
        analysis = self.scorer.analyze_plugin("safe_plugin", safe_code, plugin_file.parent)
...
'''
        ...
        analysis = self.scorer.analyze_plugin("unsafe_plugin", unsafe_code, plugin_file.parent)
...
'''
        ...
        analysis = self.scorer.analyze_plugin("complex_plugin", complex_code, plugin_file.parent)
        ...
        return {
            ...
            "issues_count": len(analysis['safety_analysis']['issues']),
            "warnings_count": len(analysis['safety_analysis']['warnings']),
            "passed": analysis['confidence_score'] > 0.5 and analysis['safety_analysis']['risk_level'] in ['LOW', 'MEDIUM']
        }
...
'''
        ...
        analysis = self.scorer.analyze_plugin("broken_plugin", broken_code, plugin_file.parent)
    ...
    def test_runtime_metrics(self) -> Dict:
        ...
        return {
            ...
            "passed": (
                metrics['total_executions'] == 13 and
                abs(metrics['success_rate'] - 10/13) < 0.01 and
                metrics['error_frequency'] == 3/13
            )
        }
...
'''
        ...
        load_result = integration.load_plugin_with_confidence_check("safe_plugin", safe_file)
...
'''
        ...
        unsafe_result = integration.load_plugin_with_confidence_check("unsafe_plugin", unsafe_file)
        ...
        return {
            ...
            "passed": (
                load_result.get('loaded', False) and
                unsafe_result.get('blocked', False)
            )
        }
