...
class WarningAnalyzer:
    ...
    def categorize_warning(self, warning):
        ...
        if any(
            keyword in message
            for keyword in ["unused import", "unused variable", "unused argument"]
        ):
        ...
        # Syntax and style warnings
            ...
            keyword in message
            for keyword in ["invalid syntax", "syntax error", "indentation"]
        ):
        ...
        # Type hints
            ...
            keyword in message
            for keyword in ["type hint", "missing type", "annotation"]
        ):
            self.warning_categories["type_hints"].append(warning)

        # Missing docstrings
        elif any(
            keyword in message for keyword in ["missing docstring", "undocumented"]
        ):
        ...
        # Code style (PEP 8, naming, etc.)
            ...
            keyword in message
            for keyword in ["naming convention", "line too long", "whitespace", "pep 8"]
        ):
        ...
        # Security issues
            ...
            keyword in message
            for keyword in ["security", "unsafe", "injection", "eval"]
        ):
            self.warning_categories["security_issues"].append(warning)

        # Performance issues
        elif any(
            keyword in message for keyword in ["performance", "inefficient", "slow"]
        ):
                        ...
                        custom_warnings.append(
                            {
                                ...
                                "category": "code_style"
                                if pattern_name in ["long_lines", "print_statements"]
                                else "other",
                            }
                        )
    ...
    def generate_report(self):
        ...
        category_counts = {
            cat: len(warnings) for cat, warnings in self.warning_categories.items()
        }
    ...
    def print_summary_report(self, report):
        ...
        print(
            f"🟡 Medium Priority: {report['priority_analysis']['medium']:4d} warnings"
        )
        ...
        for category, count in sorted(
            ...
            if count > 0:
                ...
                priority = (
                    "🔴"
                    if category
                    in ["syntax_warnings", "potential_bugs", "security_issues"]
                    else "🟡"
                    if category
                    in ["unused_imports", "unused_variables", "deprecation_warnings"]
                    else "🟢"
                )
        ...
        if high_priority > 0:
            print(
                f"🔴 URGENT: Address {high_priority} high-priority warnings immediately"
            )
    ...
    def run_analysis(self):
        ...
        try:
            ...
            print(
                f"✅ Custom pattern analysis complete: {len(custom_warnings)} warnings"
            )
        except Exception as e:
