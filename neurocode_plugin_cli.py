...
def format_plugin_list(plugins, detailed=False):
    ...
    for plugin in plugins:
        ...
        # Basic info
        output.append(f"{status} {plugin.manifest.name} v{plugin.manifest.version} ({plugin.manifest.category})")
        output.append(f"    {plugin.manifest.description}")
        
        if detailed:
            output.append(f"    Author: {plugin.manifest.author} | License: {plugin.manifest.license}")
...
def cmd_install(args, manager):
    ...
    if len(args) < 1:
        ...
        print("Usage: neurocode plugin install <name> [--version=VERSION] [--force] [--skip-security]")
        return 1
...
Plugin Registry: https://registry.neurocode.org
Documentation: https://docs.neurocode.org/plugins
"""
    print(help_text)
